...
def mask_generator(bathyfile, coordfile, outfile):
    ...
    ncvar = ncOUT.createVariable("e1t", "d", ("time", "z_a", "y", "x"))
    ncvar[:] = E1t
    ncvar = ncOUT.createVariable("e2t", "d", ("time", "z_a", "y", "x"))
    ncvar[:] = E2t
    ncvar = ncOUT.createVariable("e3t", "d", ("time", "z", "y", "x"))
    ncvar[:] = E3t
    ncvar = ncOUT.createVariable("nav_lat", "f", ("y", "x"))
    ncvar[:] = nav_lat
    ncvar = ncOUT.createVariable("nav_lev", "f", ("z",))
    ncvar[:] = nav_lev
    ncvar = ncOUT.createVariable("nav_lon", "f", ("y", "x"))
    ncvar[:] = nav_lon
    ncvar = ncOUT.createVariable("glamt", "d", ("time", "z_a", "y", "x"))
    ncvar[:] = nav_lon
    ncvar = ncOUT.createVariable("gphit", "d", ("time", "z_a", "y", "x"))
    ncvar[:] = nav_lat
    ncvar = ncOUT.createVariable("tmask", "d", ("time", "z", "y", "x"))
    ncvar[:] = tmask
    ncOUT.close()
