...
def getFilename(INPUTDIR, date17):
    ...
    for days in range(0, 3):
        d = D - DL.relativedelta(days = days)
        hours = D.hour + 24 * days + 1 # only difference with prev loop
        filename =  "%sasogs_%s00+00%02d.asc"  %( INPUTDIR, d.strftime("%Y%m%d"), hours)
        if os.path.exists(filename): 
            return filename
...
VARS=['Lat','Lon','atemp','aqh','uwind', 'vwind', 'apress', 'swdown', 'lwdown', 'precip' ]
DType=[]
...
for it,time in enumerate(TIMELIST):
    ...
    print('DEBUG', time, filename)
    try:
        ARSO=np.loadtxt(filename, dtype=DType)
        #ARSO=np.loadtxt(filename)
        ARSO.resize((jpj,jpi))
    except:
        print('****** Wrong meteo format')

    print( 'DEBUG', it, filename )
    M[it,:,:] = ARSO
